...
Space(dims::Vararg{Union{Int, Symmetry}}) = Space{dims}()
Space(dims::Tuple) = Space{dims}()
...
Base.length(::Type{StaticIndex{index}}) where {index} = length(index)
...
dropfirst() = error()
dropfirst(x::Int) = ()
dropfirst(::Symmetry{NTuple{2, dim}}) where {dim} = (dim,)
dropfirst(::Symmetry{NTuple{order, dim}}) where {order, dim} = (Symmetry{NTuple{order-1, dim}}(),)
_dropfirst(x, ys...) = (dropfirst(x)..., ys...)
dropfirst(::Space{S}) where {S} = Space(_dropfirst(S...))
droplast(::Space{S}) where {S} = Space(reverse(_dropfirst(reverse(S)...)))
...
function _promote_space(x::Tuple{Vararg{Union{Int, Symmetry}}}, y::Tuple{Vararg{Union{Int, Symmetry}}}, promoted::Tuple)
    x1 = x[1]
    y1 = y[1]
    if x1 == y1
        # just use `x1`
