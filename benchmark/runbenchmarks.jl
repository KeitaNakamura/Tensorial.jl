...
let path = "../docs/src/Benchmarks.md"
    ...
    open(path, "w") do file
        ...
        print(file,
              ...
              The CI-generated table below compares common small-tensor operations for
              `Tensor`, `Array`, and `SArray`. It is meant to show that Tensorial stays in the
              same small-tensor performance range as `SArray` while working directly with
              tensor types.

              For fourth-order tensor baselines, the `Array` and `SArray` implementations use
              the classical [Voigt form](https://en.wikipedia.org/wiki/Voigt_notation) so that
              symmetries are handled correctly. Tensorial performs the same operations on
              tensor types directly, without manually converting formulas to Voigt form.
        ...
        print(file,
              ...
              [`runbenchmarks.jl`](https://github.com/KeitaNakamura/Tensorial.jl/blob/main/benchmark/runbenchmarks.jl)
